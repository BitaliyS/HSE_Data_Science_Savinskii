...
-- оценки должны быть в диапазоне от 1 до 5) — 1 балл;" Я сделал приближенное к реальности в ВШЭ и оценки стоят от 1 до 10
-- Также отмечу, что учусь я нормально, не обращайте внимание, что в БД у меня самая низкая средняя оценка))
CREATE TABLE grades (
    id INT AUTO_INCREMENT PRIMARY KEY,
